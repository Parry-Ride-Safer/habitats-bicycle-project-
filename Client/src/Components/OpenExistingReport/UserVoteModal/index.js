  ...
  return (
    <div className="vote-layout">
      <div className="edit-vote-display">
        <p>{
             ((Number(getReportData.voting).toFixed(2) <=1.29) ? (<div className="avg-vote-icon svg">{dangerLevel[0].icon} </div>) :
             (
               (Number(getReportData.voting).toFixed(2) >= 1.30) && 
               (Number(getReportData.voting).toFixed(2) <=2.29) 
             ) ?  (<div className="avg-vote-icon svg">{dangerLevel[1].icon} </div>) :  (<div className="avg-vote-icon svg">{dangerLevel[2].icon} </div>))
        }</p>
